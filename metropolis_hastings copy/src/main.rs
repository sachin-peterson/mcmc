use crate::metropolis_hastings::metropolis_hastings;
use rayon::prelude::*;

mod metropolis_hastings;

fn main() {
    // Input parameters for Metropolis-Hastings
    let x0 = 0.0;
    let n = 1_000_000;
    ...
    // Run independent MH chains in parallel
    let chains: Vec<Vec<f64>> = (0..m).into_par_iter()
        .map(|_| metropolis_hastings(x0, n, mean_shift, proposal_std))
        .collect();
